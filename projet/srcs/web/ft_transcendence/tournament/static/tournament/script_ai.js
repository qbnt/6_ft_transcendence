function startAiPongGame() {
        ...
        update: function () {
            ...
            if (this.player1.score === rounds[this.round]) {
                if (!rounds[this.round + 1] || this.player1.round_score + 1 === 3) {
                    ...
                    sendPongResult(this.player1.round_score, this.player2.round_score);
                } else {
    ...
    // l.310 | requestAnimationFrame(fonction); fonction native du navigateur qui appelle la fonction en parametre avant le rafraichissement de l'ecran
        ...
        listen: function () {
            ...
            function updatePlayerMovement() {
                // if (keys[38]) Pong.player1.move = DIRECTION.UP;
                // else if (keys[40]) Pong.player1.move = DIRECTION.DOWN;
                // else Pong.player1.move = DIRECTION.IDLE;
                
                if (keys[87] || keys[38]) Pong.player1.move = DIRECTION.UP;
                else if (keys[83] || keys[40]) Pong.player1.move = DIRECTION.DOWN;
                else Pong.player1.move = DIRECTION.IDLE;
            }
        },
    ...
    function sendPongResult(player1Score, player2Score) {
		const pongId = pongContainer.getAttribute('data-pong-id');

		const data = new URLSearchParams();
		data.append('pong_id', pongId);
        ...
        fetch('/tournament/save_pong_result/', {
			method: 'POST',
			headers: {
